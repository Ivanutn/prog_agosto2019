...
/** \brief FILTRA LOS ELEMENTOS DE UNA LISTA Y LOS TRANSLADA A UNA NUEVA LISTA.
 *
 * \return LinkedList* RETORNA UNA LISTA CON LOS ELEMENTOS TRANSLADADOS.
 *
 */
LinkedList* ll_filter(LinkedList* this, int (*pFunc)(void*))
{
    LinkedList* auxLl = ll_newLinkedList();
    if (this != NULL && pFunc != NULL && auxLl != NULL)
    {
        for(int i = 0; i< ll_len(this); i++)
        {
            if(pFunc(ll_get(this, i)))
            {
                ll_add(auxLl, ll_get(this, i));
            }
        }
    }
    return auxLl;
